/*
  # Fix Plan System and Type Compatibility

  1. Functions
    - Create get_plan_limits function with proper type handling
    - Update validation functions to use new plan system
  
  2. Data Migration
    - Normalize plan IDs to use string identifiers
    - Ensure all users have valid plans
  
  3. Views
    - Create user_plan_details view with proper type casting
...
-- Create a function to get plan limits safely with proper type handling
...
BEGIN
  ...
  SELECT p.max_stores INTO max_stores FROM get_plan_limits(COALESCE(user_plan, 'gratuito')) p;
...
BEGIN
  ...
  SELECT p.max_products INTO max_products FROM get_plan_limits(COALESCE(user_plan, 'gratuito')) p;
...
BEGIN
  ...
  SELECT p.max_categories INTO max_categories FROM get_plan_limits(COALESCE(user_plan, 'gratuito')) p;
...
DO $$
BEGIN
  -- Update users with UUID plan IDs to use string IDs based on level
  UPDATE users
  SET plan = CASE 
    WHEN plan LIKE '%-%-%-%-%' THEN 'gratuito'
    ELSE plan
  END
  WHERE plan LIKE '%-%-%-%-%' OR plan IS NULL OR plan = '';
  
  -- Ensure all users have a valid plan
  UPDATE users
  SET plan = 'gratuito'
  WHERE plan IS NULL OR plan = '' OR plan NOT IN (
    SELECT id::text FROM plans WHERE is_active = true
    UNION ALL
    SELECT 'gratuito'
    UNION ALL
    SELECT 'emprendedor'
...
-- Create a view to join users with their plan details using proper type casting

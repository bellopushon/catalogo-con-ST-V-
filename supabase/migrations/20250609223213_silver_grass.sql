/*
  # Complete Database Schema Setup with Safe Type Creation

  1. Database Schema
    - Create all necessary types (with safe IF NOT EXISTS equivalent)
...
-- Create types safely (equivalent to IF NOT EXISTS)
DO $$ BEGIN
...
-- Tabla de logs del sistema (solo para super admin)
...
-- Políticas para users (drop existing first to avoid conflicts)
...
DROP POLICY IF EXISTS "Users can create categories for own stores" ON categories;
...
DROP POLICY IF EXISTS "Users can create products for own stores" ON products;
...
-- Políticas para system_logs (solo super admin)
DROP POLICY IF EXISTS "Super admin can manage system logs" ON system_logs;

CREATE POLICY "Super admin can manage system logs" ON system_logs FOR ALL TO authenticated USING (
  EXISTS (
    SELECT 1 FROM users 
...
-- Triggers para actualizar updated_at (drop existing first)
...
-- Triggers para validar límites (drop existing first)

...
-- Crear vista para facilitar consultas de planes SIN comparaciones UUID problemáticas
CREATE OR REPLACE VIEW user_plan_details AS
WITH user_plan_mapping AS (
  SELECT 
    u.id as user_id,
    u.email,
    u.name,
...
plan_details AS (
  SELECT 
    ...
    p.is_free
  FROM user_plan_mapping upm
  LEFT JOIN plans p ON (
    -- Usar solo comparaciones de texto seguras
    CASE 
      WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'plans' AND column_name = 'id' AND data_type = 'uuid'
      ) THEN (
        p.id::text = upm.safe_plan_id
      )
      ELSE (
        p.id = upm.safe_plan_id
      )
    END
  )
)
...
COMMENT ON VIEW user_plan_details IS 'Vista que combina información de usuarios y sus planes sin comparaciones UUID problemáticas';
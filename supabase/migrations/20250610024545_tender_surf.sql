...
-- Elimina COMPLETAMENTE las comparaciones UUID vs TEXT problemáticas
...
-- Crear función auxiliar para obtener plan_id de forma segura SIEMPRE como texto
...
BEGIN
  ...
  IF plans_id_type = 'uuid' THEN
    -- Si plans.id es UUID, SIEMPRE devolver como texto
...
-- Crear función auxiliar para obtener datos completos del plan
CREATE OR REPLACE FUNCTION get_plan_by_identifier(plan_identifier text)
RETURNS plans AS $$
DECLARE
  result_plan plans;
  plans_id_type text;
...
-- Crear vista para facilitar consultas de planes SIN NINGUNA comparación UUID problemática
CREATE OR REPLACE VIEW user_plan_details AS
WITH plan_ids AS (
  -- Primero obtenemos todos los IDs de planes como texto
  SELECT 
    id,
    CASE 
      WHEN EXISTS (
...
user_plans AS (
  -- Luego mapeamos usuarios a sus planes usando get_safe_plan_id
  SELECT 
    u.id as user_id,
    u.email,
    u.name as user_name,
    u.plan as plan_identifier,
    get_safe_plan_id(u.plan) as safe_plan_id,
    u.subscription_status,
    u.subscription_start_date,
    u.subscription_end_date
  FROM users u
)
-- Finalmente unimos todo usando solo comparaciones de texto
SELECT 
  up.user_id,
  up.email,
  up.user_name,
  up.plan_identifier,
  up.safe_plan_id as plan_id,
  p.name as plan_name,
...
FROM user_plans up
LEFT JOIN plan_ids p ON p.id_text = up.safe_plan_id OR p.plan_code = up.safe_plan_id;
...
COMMENT ON FUNCTION get_plan_by_identifier(text) IS 'Obtiene los datos completos de un plan por identificador';
...
COMMENT ON VIEW user_plan_details IS 'Vista que combina información de usuarios y sus planes sin ninguna comparación UUID problemática';